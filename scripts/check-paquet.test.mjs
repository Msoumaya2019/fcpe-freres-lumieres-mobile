...
/**
 * Écrit un bundle dans un dossier temporaire et lance le contrôle dessus.
 *
 ...
 */
function eprouver({ bundle, url = ADRESSE, clef = CLEF_PUBLIQUE }) {
  const dossier = mkdtempSync(join(tmpdir(), 'fcpe-paquet-'));

  try {
    const chemin = join(dossier, 'main.jsbundle');
